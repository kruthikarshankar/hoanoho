<?
	...
	} else if($maxsize_unit[0] == "M") {
		$calcfactor = (1024*1024);
		$maxsize_unit_human = "MByte";
	} else if($maxsize_unit[0] == "G") {
	...
	else if ($_POST['cmd'] == "deletefile" && isset($_POST['sid'])) 
	{
		$sql = "DELETE FROM sharedfiles where sid = " . $_POST['sid'];
		mysql_query($sql);
	}
	else if($_GET['cmd'] == "extendfile" && strlen($_GET['sid']) > 0) 
	{
	...
	<? } else { ?>
		<section class="main">
			<span><h1>Datei bereitstellen</h1></span>
				...
				<div id="edit">
					<div id="space">&nbsp;</div>
					<? if(!isset($_GET['r'])) { ?>
						<div id="text">Datei auswählen:</div><div id="value"><input id="file" name="file" type="file"> (Max. <? echo $maxsize_value[0]." ".$maxsize_unit_human; ?>)</div>
						<div id="text">Passwort für Zugriff:</div><div id="value"><input id="password" name="password" value="<? echo utf8_encode($report->name); ?>"></div>
						<div id="space">&nbsp;</div>
					...
					<? } else { ?>
						<? if($_GET['r'] == 1) { ?>
							<div id="message">Die Datei konnte nicht bereitgestellt werden. Eventuell überschreitet sie die maximale Dateigröße von <? echo $maxsize_value[0]." ".$maxsize_unit_human; ?></div>
						<? } else if($_GET['r'] == 0) { ?>
							<div id="message">Die Datei wurde erfolgreich bereitgestellt: <br><br><a target="_blank" href="<? echo "http://".$__CONFIG['sharefile_remoteaddress']."/?f=".$_GET['h']; ?>"><? echo "http://".$__CONFIG['sharefile_remoteaddress']."/?f=".$_GET['h']; ?></a></div>
							<div id="space">&nbsp;</div>
							<div id="space">&nbsp;</div>
		...
		<section class="main">
			...
			if($nooresults > 0) {
				...
				?>
					<div id="listitem">
						<div id="name"><a href="<? echo "http://".$__CONFIG['sharefile_remoteaddress']."/?f=".$file->Hash; ?>" target="_blank" title="<? echo "Dateityp: ".$file->File_Type."\nDateigröße: ".$file->File_Size." Byte"; ?>"><? echo utf8_decode($file->File_Name); ?></a></div>
						<div id="status"><? echo $status.$protected; ?></div>
						<div id="counter"><? echo $file->accesscounter; ?></div>
						<div id="lastaccess"><? echo $LastAccessDate; ?></div>
						<div id="action">
								<form method="POST" enctype="multipart/form-data" name="deleteFileForm<? echo $file->SID; ?>" id="deleteFileForm">
									<input type="hidden" name="cmd" value="deletefile">
									<input type="hidden" name="sid" value="<? echo $file->SID; ?>">
								</form>
								<a href="javascript:copyToClipboard('<? echo "http://".$__CONFIG['sharefile_remoteaddress']."/?f=".$file->Hash; ?>')" title="Link kopieren"><img src="./img/link.png"></a>&nbsp;&nbsp;&nbsp;
								<a href="sharefile.php?sid=<? echo $file->SID; ?>" title="Zugriffsübersicht"><img src="./img/log.png"></a>&nbsp;&nbsp;&nbsp;
								<a href="sharefile.php?cmd=extendfile&sid=<? echo $file->SID; ?>" title="Zugriff verlängern"><img src="./img/extend_file.png"></a>&nbsp;&nbsp;&nbsp;
								<a href="javascript:document.deleteFileForm<? echo $file->SID; ?>.submit()" title="Bereitstellung aufheben" onclick="javascript:return confirm('Soll die Bereitstellung für die Datei \'<? echo utf8_encode($file->File_Name); ?>\' wirklich aufgehoben werden ?');"><img src="./img/delete.png"></a>
						</div>
					</div>
				<?
				}

...
<?
	...
	else if(isset($_POST['cmd']) && $_POST['cmd'] == "saveusersettings") {
	...
	else if(isset($_POST['cmd']) && $_POST['cmd'] == "savemailserver") {
	...
	else if(isset($_POST['cmd']) && $_POST['cmd'] == "addlink") {
		$sql = "INSERT INTO pinboard_links (uid) values (0)";
		mysql_query($sql);
	}
	else if(isset($_POST['cmd']) && $_POST['cmd'] == "deletelink") {
		$sql = "delete from pinboard_links where link_id = " . $_POST['link_id'];
		mysql_query($sql);
	}
	else if(isset($_POST['cmd']) && $_POST['cmd'] == "editlink") {
		$uid = 0;
...
<html>
	<head>
		...
		<title><? echo $__CONFIG['main_sitetitle'] ?> - Einstellungen - Personalisierung</title>
	</head>
	...
	<body>
		...
		<section class="main_configuration_personal">
			...
			<form method="POST" enctype="multipart/form-data" name="editUserSetttings" id="editForm">
				<div id="listitem">
					...
					<div id="value"><? displayNoteColors($current_notecolor); ?></div>
				</div>
				<br>
				<div id="listitem_container_left">
					<div id="listitem">
						<div id="text">Pushover Usertoken:</div>
						<? $configResult = mysql_fetch_assoc(mysql_query("SELECT pushover_usertoken value from usersettings where uid = " . $_SESSION['uid'])); ?>
						<div id="value"><input type="text" id="pushover_usertoken" name="pushover_usertoken" value="<? echo $configResult['value']; ?>"></div>
					</div>
					<div id="listitem">
						<div id="text">Pushover Apptoken:</div>
						<? $configResult = mysql_fetch_assoc(mysql_query("SELECT pushover_apptoken value from usersettings where uid = " . $_SESSION['uid'])); ?>
						<div id="value"><input type="text" id="pushover_apptoken" name="pushover_apptoken" value="<? echo $configResult['value']; ?>"></div>
					</div>
				</div>
		...
		<section class="main_configuration_personal">
			...
			?>
			<div id="listitem">
				<form method="POST" enctype="multipart/form-data" name="editlinkForm<? echo $link->link_id; ?>" id="editForm">
					<div id="linkname"><input type="text" name="linkname" value="<? echo $link->name; ?>"></div>
					<div id="linkurl"><input type="text" name="linkurl" value="<? echo $link->url; ?>"></div>
					<? if($_SESSION['isAdmin'] == 1) { ?>
					<div id="linkprivate"><input type="checkbox" name="linkprivate" <? if ($link->uid > 0) echo "checked"; ?> ></div>
					<? } else { ?>
					<div id="linkprivate"><input type="hidden" name="linkprivate" value="1"></div>
					<? } ?>
					<input type="hidden" name="cmd" value="editlink">
					<input type="hidden" name="link_id" value="<? echo $link->link_id; ?>">
				</form>
				<div id="action">
					<form method="POST" enctype="multipart/form-data" name="deletelinkForm<? echo $link->link_id; ?>" id="deleteForm">
						<input type="hidden" name="cmd" value="deletelink">
						<input type="hidden" name="link_id" value="<? echo $link->link_id; ?>">
					</form>
					<a href="#" onclick="javascript:document.editlinkForm<? echo $link->link_id; ?>.submit()" title="Änderungen speichern"><img src="./img/save.png"></a>
					&nbsp;
					<a href="javascript:document.deletelinkForm<? echo $link->link_id; ?>.submit()" title="Benutzer löschen" onclick="javascript:return confirm('Soll der Link \'<? echo $link->name; ?>\' wirklich gelöscht werden ?');"><img src="./img/delete.png"></a>
				</div>
			</div>
			<?
			}
		...
		<section class="main_configuration_personal">
			...
			<form method="POST" enctype="multipart/form-data" name="editMailserverForm" id="editMailserverForm">
				<div id="listitem">
					...
					<div id="value"><? displayMailserverTypes($mailserver->mailserver_type); ?>&nbsp;</div>
				</div>
				<div id="listitem">
					<div id="text">Mailserver:</div>
					<div id="value"><input type="text" name="mailserver_host" value="<? echo $mailserver->mailserver_host; ?>"></div>
				</div>
				<div id="listitem">
					<div id="text">Port:</div>
					<div id="value"><input type="text" name="mailserver_port" value="<? echo $mailserver->mailserver_port; ?>"></div>
				</div>
				<div id="listitem">
					<div id="text">Verschlüsselung:</div>
					<div id="value"><? displayMailserverEncryption($mailserver->mailserver_encryption); ?>&nbsp;</div>
				</div>
				<div id="listitem">
					<div id="text">Login:</div>
					<div id="value"><input type="text" name="mailserver_login" value="<? echo $mailserver->mailserver_login; ?>"></div>
				</div>
				<div id="listitem">
					<div id="text">Passwort:</div>
					<div id="value"><input type="password" name="mailserver_password" value="<? echo $mailserver->mailserver_password; ?>"></div>
				</div>
		...
		<section class="main_configuration_personal">
			...
			<form method="POST" enctype="multipart/form-data" name="editPasswordForm" id="editPasswordForm">
				...
				<div id="listitem">
					<div id="errormsg"><center><? echo $errormsg; ?></center></div>
				</div>
				<div id="listitem">
					<div id="successmsg"><center><? echo $successmsg; ?></center></div>
				</div>

...
cmd ']) && $_POST['
cmd '] == "addplan") {
		$sql = "INSERT INTO scheduler (interval_type_id, interval_time) values (1, '
00: 00 ')";
		$result = mysql_query($sql);
	}
	else if(isset($_POST['
...
isActive ']))
		...
		$sql = "update scheduler set days = '
" . $days . "
', isActive = " . $isActive . ", dev_id = " . $device . ", dev_state = '
" . $_POST['dev_state'] . "
', interval_time = '
".$_POST['interval_time']."
' where sch_id = " . $_POST['
...
uid '])
		{
			$sql = "delete from scheduler where sch_id = ".$_POST['
sch_id '];
			mysql_query($sql);
		}
	}
...
href = "./css/configuration-android.css"
type = "text/css"
media = "screen"
title = "no title"
charset = "UTF-8" > ';
		else if(strpos($_SERVER['
HTTP_USER_AGENT '], "iPhone") || strpos($_SERVER['
HTTP_USER_AGENT '], "iPad"))
			echo ' < link rel = "stylesheet"
href = "./css/configuration-ios.css"
type = "text/css"
media = "screen"
title = "no title"
charset = "UTF-8" > ';
		...
		<title><? echo $__CONFIG['
main_sitetitle '] ?> - Einstellungen - Zeitplaner</title>
	</head>
<body>
	...
	<section class="main_configuration_scheduler">
		...
		?>
		<div id="listitem">
			<form method="POST" enctype="multipart/form-data" name="editPlanForm<? echo $scheduler->sch_id; ?>" id="editForm">
				<div id="days">
					<div id="weekday">
						<? displayDays($scheduler->days,false); ?>
					</div>
					&nbsp;&nbsp;<? displayDays($scheduler->days,true); ?>
				</div>
				<div id="interval<? echo $scheduler->sch_id ?>"><input id="interval_time" name="interval_time" maxlength="5" value="<? echo $scheduler->interval_time; ?>"></div>
				<div id="device"><? displayDevices($scheduler->dev_id,$scheduler->sch_id); ?></div>
				<div id="devicestate<? echo $scheduler->sch_id ?>"></div>
				<div id="isActive"><input type="checkbox" name="isActive" <? if ($scheduler->isActive == 1) echo "checked"; ?>></div>
				<input type="hidden" name="cmd" value="editplan">
				<input type="hidden" name="sch_id" value="<? echo $scheduler->sch_id; ?>">
			</form> 
			<div id="action">
				<form method="POST" enctype="multipart/form-data" name="deletePlanForm<? echo $scheduler->sch_id; ?>" id="deleteForm">
					<input type="hidden" name="cmd" value="deleteplan">
					<input type="hidden" name="sch_id" value="<? echo $scheduler->sch_id; ?>">
				</form>
				<a href="#" onclick="javascript:document.editPlanForm<? echo $scheduler->sch_id; ?>.submit()" title="Änderungen speichern"><img src="./img/save.png"></a>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="javascript:document.deletePlanForm<? echo $scheduler->sch_id; ?>.submit()" title="Zeitplan löschen" onclick="javascript:return confirm('
Soll der Zeitplan wirklich gelö scht werden ? ');"><img src="./img/delete.png"></a>
			</div>
		</div>
		<?
			echo "<script type=\"text/javascript\">displayDeviceStates('
".$scheduler->dev_id.";
".$scheduler->typename."
',".$scheduler->sch_id.",'
".$scheduler->dev_state."

...
<?
	...
	else if ($_POST['cmd'] == "editdata" && isset($_POST['rd_id'])) 
	{
		$temp = explode(" ", $_POST['savedate']);
		$date = $temp[0];
		$time = $temp[1];
	...
	else if ($_POST['cmd'] == "deletedata" && isset($_POST['rd_id'])) 
	{
...
<body>
	...
	<section class="main_report_graph">
		<div id="headline"><h1><span><? echo $reportname; ?></span></h1></div><div id="reportyear"><form method="POST" enctype="multipart/form-data" name="yearForm" id="reportYearForm"><? displayReportYears($reportyear, $reporttype, $_GET['rid']); ?></form></div><div id="range"><form method="POST" enctype="multipart/form-data" name="rangeForm" id="rangeForm"><? displayRange($range, $reporttype); ?></form></div>
			...
			if($reporttype == "manual") {
			?>
			<h2>Grafik - letzte <? echo $range; ?> Einträge</h2>
			<table id="flot_200">
				<tr>
			...
			if($reporttype == "manual") {
			?>
			<h2>Errechnete Werte - letzte <? echo $range; ?> Einträge</h2>
			<?
			...
			<div id="header">
				...
				<div id="listitem">
					...
					<form method="POST" enctype="multipart/form-data" name="saveDataForm" id="saveDataForm">
						<div id="date"><input name="savedate" value="<? echo date('d.m.Y H:i:s'); ?>"></div>

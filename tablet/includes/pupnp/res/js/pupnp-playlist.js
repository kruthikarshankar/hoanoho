...
function UPnPPlaylist() {
        ...
        upnp.backend.call(upnp.gui.dstDevice.getUid(), 'getPlaylist', null, function(res) {
            ...
            if(Object.keys(upnp.playlist.items).length > 0) {
                ...
                for(var uid in upnp.playlist.items) {
                    ...
                    $(row).append($('<td width="32">' + removeLink + '</td>'));
                    $(row).append($('<td width="32"><img src="res/images/icons/mime/' + item.classname + '.png" alt="' + item.classname + '" alt="' + item.classname + '"/></td>'));
                    $(row).append($('<td>' + item.name + '</td>'));
                    $(row).append($('<td width="32">' + playLink + '</td>'));
    ...
    this.appendAndStart = function(deviceUid, objectId, name, classname) {
        ...
        var doIt = false;
        if(Object.keys(this.items).length == 0 || confirm(upnp.gui.i18n('Die aktuelle Playlist wird gelöscht, fortfahren ?'))) {
